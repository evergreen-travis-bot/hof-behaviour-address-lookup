...
const changeValidate = (optional) => {
  let validateItems = ['required', 'postcode'];

  if (optional) {
    validateItems = ['postcode'];
  }
  return validateItems;
};

const getFields = (key, t, validator) => ({
  ...
  return superclass => class extends superclass {
    ...
    configure(req, res, callback) {
      ...
      _.merge(req.form.options, {
        subSteps,
        addressKey,
        fields: _.pick(getFields(addressKey, req.translate, changeValidate(optional)), step.fields),
        apiError: null
      });
      if (step.template) {
    ...
    getNextStep(req, res, callback) {
      ...
      const addressFields = getFields(addressKey, req.translate, changeValidate(optional));
      const addressRequired = _.find(addressFields[`${addressKey}-postcode`].validate, 'required');

      if (req.query.step === 'postcode' && req.form.values[`${addressKey}-postcode`] && !addressRequired) {
